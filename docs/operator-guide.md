...
2. Register the client's intake: business type, target public function,
   prior filing history in Denmark if any.
3. Run the advisor in read-only mode against udbud.dk.
4. Compare the checklist against the client's current documentation
   (CVR number via the Virk portal).
...
- named referral relationship with Danish-licensed counsel or a
...
a working referral relationship with Danish-licensed counsel or a
registered agent for whatever licensed representation the law of
Denmark requires for actual public-procurement filings.
