# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Denmark

## Classification

- Repository: `cloud-itonami-iso3166-dnk`
- ISO 3166: `DNK` (Denmark)
...
- an already-incorporated `cloud-itonami-cofog-{code}` /
  ...
  `cloud-itonami-{ISIC}` operator wanting to bid on a Danish
  public contract
...
- registration walkthrough for udbud.dk, Denmark's public-procurement
  portal operated by the Public Procurement Division of the Danish
  Competition and Consumer Authority
- business/tax registration checklist: a CVR number (Central Business
  Register) via the Virk portal, operated by the Danish Business
  Authority under the Ministry of Business and Growth
- local-content / preferential-procurement navigation: EU-wide open
  ...
  member state), but a valid CVR number and Danish-language submission
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Danish-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Denmark).
