(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Denmark-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Denmark-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-eu-establishment?
                                       true` (near-universal for Denmark
                                       ...
                                       Grounded in EU establishment or Danish
                                       CVR entity for many procedures
                                       (contracting authorities / KFST).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(CVR/統一資格/Udbud.dk登録/代理人確認等)が充足していない状態での提案"}]))))
...
(defn- eu-establishment-missing-violations
  ...
  `:requires-eu-establishment?` ground truth (most Denmark public
    ...
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-eu-establishment? e))
        ...
        [{:rule :eu-establishment-missing
          :detail (str subject " はデンマーク居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- cvr-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-cvr? true`, INDEPENDENTLY check
  `:cvr-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-cvr? e))
                 (not (true? (:cvr-verified? e))))
        [{:rule :cvr-unverified
          :detail (str subject " はCVR確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (cvr-unverified-violations request st)
